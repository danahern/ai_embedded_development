...
APPS_URL=$(git -C firmware remote get-url origin)
...
git submodule add "$APPS_URL" firmware

# Clean up backups
echo "Cleaning up..."
rm -rf claude-config.bak claude-mcps.bak firmware.bak

echo ""
echo "=== Done ==="
echo "Submodules added. Commit with:"
echo "  git add .gitmodules claude-config claude-mcps firmware"
